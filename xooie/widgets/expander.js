...
define('xooie/widgets/expander', ['jquery', 'xooie/base'], function($, Base) {
    var Expander;
   ...
   var parseWhich = function(which) {
    ...
    Expander = Base.extend(function(){

    });

// PROPERTIES

    Expander.define('triggers');

    Expander.define('namespace', 'expander');

    Expander.defineReadOnly('handleSelector', '[data-role="dropdown-handle"]');

    Expander.defineReadOnly('expanderSelector', '[data-role="dropdown-content"]');

    Expander.defineReadOnly('activeClass', 'is-active');

    Expander.prototype._timers = {
        expand: [],
        collapse: []
    };
...
// PROTOTYPE METHODS
    ...
    Expander.prototype._setState = function(index, active, options) {
        if (typeof index === 'undefined' || isNaN(index)) {
            return;
        }

        var state, countersState;
    ...
    Expander.prototype._process_triggers = function(triggers) {
        if (typeof triggers.on === 'undefined') {
            triggers.on = { focus: { delay: 0 } };
        }

        if (typeof triggers.off === 'undefined') {
            triggers.off = { blur: { delay: 0 } };
        }
